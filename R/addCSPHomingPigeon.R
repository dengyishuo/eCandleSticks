...
#'
#' Identifies the Homing Pigeon: two black candles where the second is inside the first's body.
#'
...
#' \itemize{
#' \item HomingPigeon: TRUE if pattern detected
#' }
#'
#' @family pattern-2bar
#' @family pattern-bull
...
#' @export
addCSPHomingPigeon <- function(x, output = c("xts", "tibble", "data.frame")) {
  if (!xts::is.xts(x)) {
  ...
  colnames(result) <- c("HomingPigeon")
