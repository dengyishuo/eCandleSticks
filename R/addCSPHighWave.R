...
#'
#' Identifies High-Wave candles: small body with very long shadows on both sides.
#'
#' @param x xts or data.frame with OHLC price data
#' @param maxbodyCL Maximum body ratio to candle length. Default 0.1.
#' @param minshadowCL Minimum shadow ratio to candle length. Default 0.3.
...
#' \itemize{
#' \item HighWave: TRUE if pattern detected
#' }
#'
#' @family pattern-1bar
...
#' @export
addCSPHighWave <- function(x, maxbodyCL = 0.1, minshadowCL = 0.3, output = c("xts", "tibble", "data.frame")) {
  if (!xts::is.xts(x)) {
