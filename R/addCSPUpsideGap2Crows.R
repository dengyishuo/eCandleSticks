...
#'
#' Identifies Upside Gap Two Crows: white candle, gap-up black, engulfing black in gap.
#'
...
#' \itemize{
#' \item UpsideGap2Crows: TRUE if pattern detected
#' }
#'
#' @family pattern-3bar
#' @family pattern-bear
...
#' @export
addCSPUpsideGap2Crows <- function(x, output = c("xts", "tibble", "data.frame")) {
  if (!xts::is.xts(x)) {
  ...
  result <- UpsideGap2Crows

  colnames(result) <- c("UpsideGap2Crows")
