...
#'
#' Identifies Breakaway: long candle, gap continuation bars, then reversal bar closing into gap.
#'
...
#' \itemize{
#' \item BullBreakaway: TRUE if pattern detected
#' \item BearBreakaway: TRUE if pattern detected
#' }
#'
#' @family pattern-5bar
#' @family pattern-bull
...
#' @export
addCSPBreakaway <- function(x, minbodyCL = 0.3, output = c("xts", "tibble", "data.frame")) {
  if (!xts::is.xts(x)) {
  ...
  Hi4 <- stats::lag(quantmod::Hi(TS), -4)
  ...
  Op0 <- quantmod::Op(TS)
  Cl0 <- quantmod::Cl(TS)
  CL4 <- Hi4 - stats::lag(quantmod::Lo(TS), -4)

  BullBreakaway <- xts::reclass(
    Op4 > Cl4 &
      (Op4 - Cl4) >= CL4 * minbodyCL &
      Lo3 < stats::lag(quantmod::Lo(TS), -4) &
      Cl3 < Cl4 &
      Cl2 < Cl3 &
      Cl1 < Cl2 &
      Cl0 > Op0 &
  ...
  BearBreakaway <- xts::reclass(
    Cl4 > Op4 &
      (Cl4 - Op4) >= CL4 * minbodyCL &
      Hi3 > Hi4 &
      Cl3 > Cl4 &
      Cl2 > Cl3 &
      Cl1 > Cl2 &
  ...
  result <- cbind(BullBreakaway, BearBreakaway)

  colnames(result) <- c("BullBreakaway", "BearBreakaway")
