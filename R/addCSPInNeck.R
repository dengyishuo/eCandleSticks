...
#'
#' Identifies In-Neck pattern: black candle then white that closes at/near Bar 1 close.
#'
#' @param x xts or data.frame with OHLC price data
#' @param tolerance Close tolerance ratio. Default 0.05.
...
#' \itemize{
#' \item InNeck: TRUE if pattern detected
#' }
#'
#' @family pattern-2bar
#' @family pattern-bear
...
#' @export
addCSPInNeck <- function(x, tolerance = 0.05, output = c("xts", "tibble", "data.frame")) {
  if (!xts::is.xts(x)) {
  ...
  InNeck <- xts::reclass(
    Op1 > Cl1 &
      ...
      Cl0 > Op0 &
      Cl0 <= Cl1 &
      Cl0 >= Cl1 - CL1 * tolerance,
    TS)

  result <- InNeck

  colnames(result) <- c("InNeck")
