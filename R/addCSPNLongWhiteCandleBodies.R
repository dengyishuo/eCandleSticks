#' Add CSP N Long White Candle Bodies Points to Candlestick Chart
#'
#' This function adds CSP N Long White Candle Bodies points to a candlestick chart
#' created by eCandleSticks using the results from a CSP N Long White Candle Bodies analysis,
...
#' @param csp_n_long_white_result The result object from CSP N Long White Candle Bodies analysis,
#'        which should be a data frame containing a 'Date' column and a logical column
#'        'X2LongWhiteCandleBodies' (or similar pattern for different N values)
#' @param n_long_white_color Color for N Long White Candle Bodies points. Default "green".
#' @param point_size Size for the N Long White Candle Bodies points. Default 3.
#' @param n_long_white_shape Shape for N Long White Candle Bodies points. Default 16 (filled circle).
#' @param point_alpha Alpha transparency for the N Long White Candle Bodies points. Default 0.8.
#' @param mark_at_close Whether to mark at the close price. Default TRUE.
#'
#' @return A modified eCandleSticks result list with N Long White Candle Bodies points added to the price plot
...
#'
#' # Get CSP N Long White Candle Bodies results
#' csp_n_long_white_data <- CSPNLongWhiteCandleBodies(AAPL) # This returns a data frame with X2LongWhiteCandleBodies column
#'
#' # Add N Long White Candle Bodies points
#' result_with_n_long_white <- addCSPNLongWhiteCandleBodies(result, csp_n_long_white_data)
#'
#' # Display the combined plot with N Long White Candle Bodies points
#' print(result_with_n_long_white$combined_plot)
#' }
addCSPNLongWhiteCandleBodies <- function(eCandleSticks_result, csp_n_long_white_result,
  ...
  # Find the column name that contains "LongWhiteCandleBodies"
  pattern_col <- grep("LongWhiteCandleBodies", colnames(csp_n_long_white_result), value = TRUE)
  if (length(pattern_col) == 0) {
    stop("csp_n_long_white_result must contain a column with 'LongWhiteCandleBodies' in its name")
  }

  # Use the first matching column
  pattern_col_name <- pattern_col[1]

  # Extract the N value from the column name (e.g., "X2" from "X2LongWhiteCandleBodies")
  n_value <- gsub("LongWhiteCandleBodies", "", pattern_col_name)
  n_value <- gsub("X", "", n_value)
  pattern_name <- paste0(n_value, " Long White Candle Bodies")
  ...
  # Extract N Long White Candle Bodies points
  ...
  # Add N Long White Candle Bodies points to the price plot
  price_plot_with_n_long_white <- eCandleSticks_result$price_plot

  # Add N Long White Candle Bodies points (if any)
  ...
  # Add color scale and legend only if there are any N Long White Candle Bodies points
  if (nrow(n_long_white_points) > 0) {
    price_plot_with_n_long_white <- price_plot_with_n_long_white +
